...
fn distance(
    ...
    let x_dist = match b.0 > a.0 {
        true => (b.0 - a.0) + (a.0..b.0).filter(|x| empty_cols.contains(x)).count() * (1000000 - 1),
        _ => (a.0 - b.0) + (b.0..a.0).filter(|x| empty_cols.contains(x)).count() * (1000000 - 1),
    };
    let y_dist = match b.1 > a.1 {
        true => (b.1 - a.1) + (a.1..b.1).filter(|x| empty_rows.contains(x)).count() * (1000000 - 1),
        _ => (a.1 - b.1) + (b.1..a.1).filter(|x| empty_rows.contains(x)).count() * (1000000 - 1),
    };
    x_dist + y_dist
}
